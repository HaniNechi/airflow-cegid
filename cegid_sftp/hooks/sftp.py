...
class SFTPHook(SSHHook):
    ...
    def describe_directory(self, path: str) -> dict[str, dict[str, str | int | None]]:
        ...
        return files
    def download_files_matching_regex(
    self,
    remote_directory: str,
    regex_pattern: str
) -> list[io.BytesIO]:
        """
        Download files from remote directory that match the regex pattern.

        :param remote_directory: Remote directory to search for files.
        :param regex_pattern: Regular expression pattern to match file names.
        :return: List of memory objects containing downloaded file contents.
        """
        try:
            ...
            for file_name in matching_files:
                ...
                memory_object = io.BytesIO(file_contents)
                memory_object.seek(0)  # Reset memory object pointer
                memory_objects.append({"memory_object": memory_object , "filename" : file_name})
            ...
            raise AirflowException(f"Error while downloading files: {e}")
